...
# --------- NEW: Surplus (only when ALL 3 dekads > threshold) ----------
...
with st.sidebar:
    ...
    st.markdown(
        "<small>Hint: For many planning exercises, a minimum flow of 5000 cusecs is used.</small>",
        unsafe_allow_html=True
    )
    st.subheader("Valuation")
    cost_per_maf = st.number_input("Cost per MAF (USD)", min_value=0.0, value=1_000_000_000.0, step=50_000_000.0,
                                   help="Hint: Some studies approximate 1 MAF ≈ USD 1 billion.")

try:
    tidy = load_matrix_from_path(excel_path, sheet_name=sheet_name)
...
with tab1:
    ...
    if c1.button("Run (All Periods)"):
        ...
        st.dataframe(res[["Period","Month","Days_in_Month","Days_Below_Threshold","Pct_Days_Below_%"]], use_container_width=True)
        ...
        st.markdown(f"""
**Summary for {month} @ {threshold} cusecs**  
- Min days below: **{vmin}** — Periods: {mins}  
- Max days below: **{vmax}** — Periods: {maxs}  
- Zero-days periods: {len(zeros)} ({", ".join(zeros) if zeros else "none"})  
- Median: **{median}**, IQR: **{q1}–{q3}**  
""")

        csv = res.drop(columns=["StartYear"]).to_csv(index=False).encode("utf-8")
        st.download_button("Download CSV", data=csv, file_name=f"{month}_below_{threshold}_by_period.csv", mime="text/csv")
...
with tab2:
    ...
    if c1.button("Run (Selected Range)"):
        ...
        if res.empty:
            ...
            median = int(np.median(vals)); q1 = int(np.percentile(vals,25)); q3 = int(np.percentile(vals,75))
            st.markdown(f"""
**Summary for {month_r} @ {threshold} cusecs  | Range: {start_p} → {end_p}**  
- Min days below: **{vmin}** — Periods: {mins}  
- Max days below: **{vmax}** — Periods: {maxs}  
- Zero-days periods: {len(zeros)} ({", ".join(zeros) if zeros else "none"})  
- Median: **{median}**, IQR: **{q1}–{q3}**  
""")
            ...
            csv = res.drop(columns=["StartYear"]).to_csv(index=False).encode("utf-8")
            st.download_button("Download CSV", data=csv, file_name=f"{month_r}_below_{threshold}_by_period_{start_p}_to_{end_p}.csv", mime="text/csv")
...
with tab3:
    ...
    if c1.button("Run (By Period)"):
        ...
        st.markdown(f"""
**Summary for {period_sel} @ threshold {threshold} cusecs**  
- Min days below: **{vmin}** — Months: {mins}  
- Max days below: **{vmax}** — Months: {maxs}  
- Months with zero days below: {len(zeros)} ({", ".join(zeros) if zeros else "none"})  
- Median: **{median}**, IQR: **{q1}–{q3}**  
- **Driest 5 months**: {", ".join(driest_5)}  
- **Wettest 5 months**: {", ".join(wettest_5)}  
""")

# --- Tab 4: NEW Surplus & Value ---
with tab4:
    st.subheader("Surplus water above threshold → MAF → Value")
    st.caption("Only months/periods where **all three ten-daily averages** exceed the threshold are counted (proxy for ‘always above requirement on daily basis’).")
    ...
    with colA:
        ...
        if st.button("Compute Surplus (Month across Periods)"):
            ...
            df = df[df["Period"].isin(allowed)].copy()
            # keep only those with all days above
            ...
            if df.empty:
                ...
                st.dataframe(df[["Period","Month","AllDaysAbove","Surplus_MAF","Value_USD"]], use_container_width=True)

                # Bar: Surplus MAF by period
                fig, ax = plt.subplots(figsize=(10,4))
                ...
                st.markdown(f"**Total surplus (MAF):** {total_maf:,.3f}  |  **Estimated value (USD):** ${total_val:,.0f}")
    ...
    st.divider()
    st.markdown(
        "<small><b>Important:</b> Using ten-daily averages as a proxy for ‘always above daily requirement’. "
        "We count a month only if <i>all three</i> dekad averages exceed the threshold. "
        "Surplus = ∑( (avg − threshold) × days_in_dekad ) and converted to MAF using 1 cusec-day ≈ 1.9835 acre-feet.</small>",
        unsafe_allow_html=True
    )
